...
// Configuração do banco para splash_slider
...
@ $db->exec("ALTER TABLE splash_slider ADD COLUMN title TEXT DEFAULT ''");
@ $db->exec("ALTER TABLE splash_slider ADD COLUMN sort_order INTEGER DEFAULT 0");
@ $db->exec("ALTER TABLE splash_slider ADD COLUMN is_active INTEGER DEFAULT 1");
@ $db->exec("ALTER TABLE splash_slider ADD COLUMN created_at TEXT DEFAULT CURRENT_TIMESTAMP");
...
function uploadSplashImage($file) {
    if (!isset($file) || !is_array($file) || (int)$file['error'] !== 0) return null;
    $allowed = ['image/png', 'image/jpeg', 'image/jpg', 'image/webp', 'image/gif'];
    if (!in_array($file['type'], $allowed, true)) return null;
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    if ($ext === 'jpeg') $ext = 'jpg';
    $name = uniqid('splash_', true) . '.' . $ext;
    $dir = 'uploads/splash/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_slider'])) {
    ...
    $uploaded = uploadSplashImage($_FILES['image_file'] ?? null);
    if (!empty($uploaded)) $imageUrl = $uploaded;

    if (!empty($imageUrl)) {
        $stmt = $db->prepare('INSERT INTO splash_slider(title, image_url, sort_order, is_active) VALUES (:title, :image_url, :sort_order, 1)');
        $stmt->bindValue(':title', $title, SQLITE3_TEXT);
        $stmt->bindValue(':image_url', $imageUrl, SQLITE3_TEXT);
        $stmt->bindValue(':sort_order', $sortOrder, SQLITE3_INTEGER);
        $stmt->execute();
    }
    header('Location: splash_slider.php');
    exit;
}

if (isset($_GET['toggle'])) {
    $id = (int)$_GET['toggle'];
    $db->exec("UPDATE splash_slider SET is_active = CASE WHEN is_active = 1 THEN 0 ELSE 1 END WHERE id = {$id}");
    header('Location: splash_slider.php');
    exit;
}
...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $db->exec("DELETE FROM splash_slider WHERE id = {$id}");
    header('Location: splash_slider.php');
    exit;
}
...
if (isset($_GET['move']) && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $direction = $_GET['move'] === 'up' ? -1 : 1;
    $current = $db->querySingle("SELECT sort_order FROM splash_slider WHERE id = {$id}");
    $current = is_numeric($current) ? (int)$current : 0;
    $newOrder = max(0, $current + $direction);
    $stmt = $db->prepare('UPDATE splash_slider SET sort_order = :sort_order WHERE id = :id');
...
$res = $db->query('SELECT * FROM splash_slider ORDER BY is_active DESC, sort_order ASC, id ASC');
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <style>
        /* Seus estilos existentes aqui (mantidos) */
        :root {
            --bg-0: #020617;
            --bg-1: #030b2a;
            --bg-2: #071238;
...
<div id="wrapper">
    <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar">
        ...
        <?php echo navItem('splash_slider.php', 'fas fa-images', 'Imagem de Entrada', $currentPage); ?>
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                <div class="glass-box mb-4">
                    <h3><i class="fas fa-images"></i> Slider da Splash</h3>
                    <p class="text-muted mb-0">As imagens cadastradas aqui vão aparecer no fundo da tela de carregamento do app.</p>
                </div>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" name="title" class="form-control" placeholder="Título da imagem">
                            </div>
                            <div class="col-md-4 mb-3">
                            ...
                            <div class="col-md-2 mb-3">
                                ...
                                <div class="upload-wrapper">
                                    <label for="image_file" class="upload-btn-icon">
                                        <i class="fas fa-upload"></i>
                                        <span>Envie</span>
                                    </label>
                                    <input type="file" name="image_file" id="image_file" accept="image/*">
                                    <div class="file-info-badge" id="fileInfo" style="display: none;">
                ...
                <div class="splash-grid">
                    <?php while ($row = $res->fetchArray(SQLITE3_ASSOC)): ?>
                        <div class="splash-card">
                            <img src="<?= htmlspecialchars($row['image_url']) ?>" alt="Splash image">
                            <div class="body">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <strong><?= htmlspecialchars($row['title'] ?: ('Imagem #' . $row['id'])) ?></strong>
                                    <?php if ((int)$row['is_active'] === 1): ?>
                                        <span class="badge-soft badge-on"><i class="fas fa-check-circle"></i> ATIVA</span>
                                    <?php else: ?>
                                        <span class="badge-soft badge-off"><i class="fas fa-circle"></i> INATIVA</span>
                                    <?php endif; ?>
                                </div>
                                <div class="text-muted small"><i class="fas fa-sort-amount-down"></i> Ordem: <?= (int)$row['sort_order'] ?></div>
                                <div class="small text-break mt-2"><i class="fas fa-link"></i> <?= htmlspecialchars($row['image_url']) ?></div>
                                <div class="control-row">
                                    <a class="btn btn-sm btn-primary" href="splash_slider.php?toggle=<?= (int)$row['id'] ?>"><i class="fas fa-power-off"></i> Ativar</a>
                                    <a class="btn btn-sm btn-info" href="splash_slider.php?id=<?= (int)$row['id'] ?>&move=up"><i class="fas fa-arrow-up"></i></a>
                                    <a class="btn btn-sm btn-info" href="splash_slider.php?id=<?= (int)$row['id'] ?>&move=down"><i class="fas fa-arrow-down"></i></a>
                                    <a class="btn btn-sm btn-danger" onclick="return confirm('Deseja remover esta imagem?');" href="splash_slider.php?delete=<?= (int)$row['id'] ?>"><i class="fas fa-trash"></i></a>
                                </div>
                            </div>
                        </div>
                    <?php endwhile; ?>
