...
<div class="container-fluid">
    <div class="page-top-card mb-4">
        <h2 class="mb-2"><span class="icon-bullet"><i class="fas fa-plus-circle"></i></span>Criar novo acesso</h2>
        <p class="mb-0 text-muted"><i class="fas fa-wand-magic-sparkles mr-2"></i>Preencha o tutorial passo a passo. O painel extrai os dados automaticamente da linha informada.</p>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header border-0 py-4">
            <h4 class="mb-0"><i class="fas fa-list-check mr-2"></i>Passos para criar o acesso</h4>
        </div>
        <div class="card-body">
            <form method="post">
                <div class="form-group mb-4">
                    <label for="mac_address"><i class="fas fa-mobile-alt mr-2"></i><strong>Passo 1 — Informe o ID do dispositivo</strong></label>
                    <input class="form-control" id="mac_address" name="mac_address" placeholder="Ex.: 00:11:22:33:44:55" type="text" maxlength="17" required value="<?php echo htmlspecialchars($_POST['mac_address'] ?? ''); ?>">
                </div>

                <div class="form-group mb-4">
                    <label for="title"><i class="fas fa-user mr-2"></i><strong>Passo 2 — Digite o nome do cliente</strong></label>
                    <input type="text" class="form-control" name="title" id="title" placeholder="Ex.: Cliente João" required value="<?php echo htmlspecialchars($_POST['title'] ?? ''); ?>">
                </div>

                <div class="form-group mb-4">
                    <label for="expire_date"><i class="fas fa-calendar-alt mr-2"></i><strong>Passo 3 — Insira a data de expiração</strong></label>
                    <input type="text" class="form-control" name="expire_date" id="datetimepicker" placeholder="YYYY-MM-DD" required value="<?php echo htmlspecialchars($_POST['expire_date'] ?? ''); ?>">
                </div>

                <div class="form-group mb-4">
                    <label for="support_contact"><i class="fab fa-whatsapp mr-2"></i><strong>Passo 4 — Contato Suporte Vendedor</strong></label>
                    <input type="text" class="form-control" name="support_contact" id="support_contact" placeholder="Ex.: 5591999999999 ou +55 (91) 99999-9999" value="<?php echo htmlspecialchars($_POST['support_contact'] ?? ''); ?>">
                    <small class="form-text text-muted"><i class="fas fa-circle-info mr-1"></i>Esse número será usado no QR da splash para abrir o WhatsApp do vendedor responsável por este cadastro.</small>
                </div>

                <div class="form-group mb-4">
                    <label for="playlist_input"><i class="fas fa-link mr-2"></i><strong>Passo 5 — Insira a lista M3U ou linha Xtream Codes</strong></label>
                    <textarea class="form-control" name="playlist_input" id="playlist_input" rows="5" placeholder="Cole aqui a URL M3U ou a linha com username e password" required><?php echo htmlspecialchars($_POST['playlist_input'] ?? ''); ?></textarea>
                    <small class="form-text text-muted"><i class="fas fa-circle-info mr-1"></i>O sistema usa apenas Xtream Codes. Ao colar a linha, ele extrai DNS, usuário e senha automaticamente.</small>
                </div>

                <div class="d-flex flex-wrap" style="gap:10px;">
                    <button class="btn btn-success" name="submit" type="submit"><i class="fas fa-check-circle mr-2"></i>Criar acesso</button>
                    <a href="users.php?tab=gerenciar" class="btn btn-secondary"><i class="fas fa-arrow-left mr-2"></i>Voltar para gerenciar</a>
                </div>
            </form>
        </div>
    </div>
